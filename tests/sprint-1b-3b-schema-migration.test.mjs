...
test('known latest-v0 migrates once with data preservation, safety backup, FK and indexes',()=>{
  ...
  try{
    ...
    const result=JSON.parse(migrated.stdout.trim().split(/\r?\n/).at(-1));assert.equal(result.profile,'latest-v0');assert.ok(result.safetyBackup);assert.equal(existsSync(path.join(backupDirectory,result.safetyBackup)),true);const manifest=JSON.parse(readFileSync(path.join(backupDirectory,`${result.safetyBackup}.manifest.json`),'utf8'));assert.equal(manifest.purpose,'migration_safety');assert.equal(manifest.schema_version,0);assert.equal(manifest.backup_basename,result.safetyBackup);assert.match(manifest.sha256,/^[0-9a-f]{64}$/);const checked=runProduction(['check-backup',result.safetyBackup],databasePath,backupDirectory);assert.equal(checked.status,0,checked.stderr);assert.equal(JSON.parse(checked.stdout.trim()).purpose,'migration_safety');
...
test('migration failure rolls back schema and data and restores foreign_keys',()=>{
  ...
  try{
    ...
    const failed=runProduction(['migrate-schema'],databasePath,backupDirectory);assert.notEqual(failed.status,0);assert.ok(readdirSync(backupDirectory).some(name=>name.includes('-backup-migration_safety-')));assert.deepEqual(semanticState(databasePath),before);
  }finally{rmSync(directory,{recursive:true,force:true})}
});
