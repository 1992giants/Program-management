...
test('transaction hardening rolls back partial business writes and audit failures',()=>{
  ...
  const source=String.raw`
    ...
    db.prepare("INSERT INTO attendance (application_id,session_id,status,note) VALUES (?,?,'참석','')").run(applicationId,'SESSION-TX-1');failAudit('certificate_create');const certificateCount=count('certificates');response=await post({action:'certificate',applicationId},cookie);assert.equal(response.status,500);assert.equal(count('certificates'),certificateCount);drop('fail_audit');
