...
test('remaining operational mutations record canonical minimized audits',()=>{
  ...
  const source=String.raw`
    ...
    assert.equal((await post({action:'certificate',participantId:'P-NOT-FOUND',sessionCount:3},cookie)).status,400);assert.equal((await post({action:'certificate',participantId:'P-APPLY',sessionCount:0},cookie)).status,400);assert.equal(auditRows('certificate_create').length,count);
    assert.equal((await post({action:'certificate',participantId:'P-APPLY',sessionCount:3,freeText:'FREE_TEXT_SECRET'},cookie)).status,200);const certificate=db.prepare('SELECT id,participant_id,session_count FROM certificates ORDER BY id DESC LIMIT 1').get(),certificateAudit=lastAudit('certificate_create');
    assert.equal(certificateAudit.entity_id,String(certificate.id));assert.deepEqual(JSON.parse(certificateAudit.after_json),{participant_id:'P-APPLY',session_count:3,issued:true});
