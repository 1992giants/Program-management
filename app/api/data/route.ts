...
import { createRun, getDatabase, getDatabasePath, hashPin } from '@/db';
...
type SafeUser={id:string;username:string;display_name:string;role:string;active:number};

function authenticatedUser(request:Request,db:DatabaseSync) {
  const token=request.headers.get('authorization')?.replace(/^Bearer\s+/i,'')||'';
  if(!token)return undefined;
  return db.prepare(`SELECT u.id,u.username,u.display_name,u.role,u.active FROM auth_sessions s JOIN staff_users u ON u.id=s.user_id WHERE s.token=? AND s.expires_at>? AND u.active=1`).get(token,new Date().toISOString()) as SafeUser|undefined;
}
function can(user:SafeUser,action:string){if(user.role==='관리자')return true;if(user.role==='출석 입력 전용')return ['attendance','attendanceBulk'].includes(action);return !['createUser','updateUser','restoreBackup','settings'].includes(action);}
...
function logChange(db:DatabaseSync, action:string, entityType:string, entityId:string|number, before:unknown, after:unknown, summary:string, requestedActor?:string) {
  const createdAt=new Date().toLocaleString('sv-SE',{timeZone:'Asia/Seoul'});
  db.prepare('INSERT INTO audit_logs (created_at,actor,action,entity_type,entity_id,before_json,after_json,summary) VALUES (?,?,?,?,?,?,?,?)').run(createdAt,requestedActor?.trim()||actorName(db),action,entityType,String(entityId),before?JSON.stringify(before):'',after?JSON.stringify(after):'',summary);
}
function backupFiles() {
  const source=getDatabasePath(), directory=path.dirname(source), extension=path.extname(source)||'.sqlite', base=path.basename(source,extension);
...
function validateBackup(filePath:string) {
  const source=getDatabasePath(), resolved=path.resolve(filePath);
  if(path.dirname(resolved)!==path.dirname(source)||!path.basename(resolved).startsWith(`${path.basename(source,path.extname(source))}-backup-`)||!existsSync(resolved)) throw new Error('현재 데이터 폴더의 백업 파일만 선택할 수 있습니다.');
...
function snapshot(currentUser?:SafeUser) {
  ...
  return {
    ...
    users: currentUser?.role==='관리자'?rows(`SELECT id,username,display_name,role,active,created_at FROM staff_users ORDER BY active DESC, display_name`):[],
...
export async function POST(request:Request) {
  try {
    ...
    if(body.action==='login'){
      const username=text(body.username).trim().toLowerCase(),user=db.prepare('SELECT id,username,display_name,role,active,pin_hash FROM staff_users WHERE username=?').get(username) as (SafeUser&{pin_hash:string})|undefined;
      if(!user||!user.active||user.pin_hash!==hashPin(username,text(body.pin)))return Response.json({error:'계정 또는 PIN이 올바르지 않습니다.'},{status:401});
      const token=randomUUID(),createdAt=new Date().toISOString(),expiresAt=new Date(Date.now()+12*60*60*1000).toISOString();db.prepare('DELETE FROM auth_sessions WHERE expires_at<=?').run(createdAt);db.prepare('INSERT INTO auth_sessions (token,user_id,expires_at,created_at) VALUES (?,?,?,?)').run(token,user.id,expiresAt,createdAt);const safeUser:SafeUser={id:user.id,username:user.username,display_name:user.display_name,role:user.role,active:user.active};return Response.json({...snapshot(safeUser),authToken:token});
    }
    const user=authenticatedUser(request,db);
    if(!user)return Response.json({error:'로그인이 만료되었습니다.',loginRequired:true},{status:401});
    const action=text(body.action);if(!can(user,action))return Response.json({error:`${user.role} 권한으로는 이 작업을 수행할 수 없습니다.`},{status:403});
    ...
    if (body.action === 'createParticipant') {
      ...
      const insertApplication = db.prepare(`INSERT OR IGNORE INTO applications (participant_id,program_id,run_id,applied_at,status) VALUES (?,?,NULL,?,'신청')`);
      for (const programId of programIds) insertApplication.run(id,programId,today);
      logChange(db,'등록','참가자',id,null,db.prepare('SELECT * FROM participants WHERE id=?').get(id),`${text(body.name)} 참가자 등록`,text(body.actor));
      ...
      logChange(db,'수정','참가자',text(body.id),before,db.prepare('SELECT * FROM participants WHERE id=?').get(text(body.id)),`${text(body.name)} 기본정보 수정`,text(body.actor));
      ...
      const run = db.prepare('SELECT r.program_id,p.capacity FROM program_runs r JOIN programs p ON p.id=r.program_id WHERE r.id=?').get(text(body.runId)) as {program_id:string;capacity:number}|undefined;
      ...
      db.prepare('UPDATE applications SET run_id=?, status=CASE WHEN status=\'신청\' THEN \'참가대기\' ELSE status END WHERE id=?').run(text(body.runId),Number(body.applicationId));
      ...
      db.prepare(`INSERT INTO applications (participant_id,program_id,run_id,applied_at,status) VALUES (?,?,?,?,'참가대기') ON CONFLICT(participant_id,program_id) DO UPDATE SET run_id=excluded.run_id,status='참가대기'`).run(text(body.participantId),run.program_id,text(body.runId),today);
    } else if (body.action === 'applicationStatus') {
      const before=db.prepare('SELECT * FROM applications WHERE id=?').get(Number(body.id));
      db.prepare('UPDATE applications SET status=? WHERE id=?').run(text(body.status),Number(body.id));
      logChange(db,'상태 변경','신청',Number(body.id),before,db.prepare('SELECT * FROM applications WHERE id=?').get(Number(body.id)),`신청 현황을 ${text(body.status)}(으)로 변경`,text(body.actor));
    } else if (body.action === 'attendance') {
      const before=db.prepare('SELECT * FROM attendance WHERE application_id=? AND session_id=?').get(Number(body.applicationId),text(body.sessionId));
      db.prepare(`INSERT INTO attendance (application_id,session_id,status,note) VALUES (?,?,?,?) ON CONFLICT(application_id,session_id) DO UPDATE SET status=excluded.status, note=excluded.note`).run(Number(body.applicationId),text(body.sessionId),text(body.status),text(body.note));
      const after=db.prepare('SELECT * FROM attendance WHERE application_id=? AND session_id=?').get(Number(body.applicationId),text(body.sessionId));
      logChange(db,'출석 입력','출석',`${body.applicationId}:${text(body.sessionId)}`,before,after,`출석 상태를 ${text(body.status)}(으)로 변경`,text(body.actor));
      ...
      try {
        ...
        logChange(db,'병합','참가자',keepId,{유지:keep,병합:source},db.prepare('SELECT * FROM participants WHERE id=?').get(keepId),`${text(source.name)} ${mergeId} 기록을 ${keepId}로 병합`,text(body.actor));
        db.exec('COMMIT');
      } catch(error){db.exec('ROLLBACK');throw error;}
    } else if (body.action === 'createUser') {
      const username=text(body.username).trim().toLowerCase(),pin=text(body.pin),role=text(body.role);if(!username||pin.length<4)throw new Error('계정명과 4자리 이상의 PIN을 입력하세요.');if(!['관리자','일반 담당자','출석 입력 전용'].includes(role))throw new Error('올바른 권한을 선택하세요.');const id=`USR-${String(Date.now()).slice(-8)}`;db.prepare('INSERT INTO staff_users (id,username,display_name,pin_hash,role,active,created_at) VALUES (?,?,?,?,?,1,?)').run(id,username,text(body.displayName),hashPin(username,pin),role,today);logChange(db,'계정 등록','사용자',id,null,{username,displayName:text(body.displayName),role},`${text(body.displayName)} 계정 생성`,user.display_name);
    } else if (body.action === 'updateUser') {
      const target=db.prepare('SELECT * FROM staff_users WHERE id=?').get(text(body.id)) as Record<string,unknown>|undefined;if(!target)throw new Error('사용자를 찾을 수 없습니다.');const nextRole=text(body.role),nextActive=body.active?1:0;if(target.role==='관리자'&&(nextRole!=='관리자'||!nextActive)){const admins=db.prepare(`SELECT COUNT(*) AS count FROM staff_users WHERE role='관리자' AND active=1`).get() as {count:number};if(admins.count<=1)throw new Error('마지막 관리자는 권한을 변경하거나 사용 중지할 수 없습니다.');}const pin=text(body.pin);if(pin&&pin.length<4)throw new Error('PIN은 4자리 이상이어야 합니다.');if(pin)db.prepare('UPDATE staff_users SET display_name=?,role=?,active=?,pin_hash=? WHERE id=?').run(text(body.displayName),nextRole,nextActive,hashPin(String(target.username),pin),text(body.id));else db.prepare('UPDATE staff_users SET display_name=?,role=?,active=? WHERE id=?').run(text(body.displayName),nextRole,nextActive,text(body.id));logChange(db,'계정 수정','사용자',text(body.id),{display_name:target.display_name,role:target.role,active:target.active},{display_name:text(body.displayName),role:nextRole,active:nextActive},`${text(body.displayName)} 계정 수정`,user.display_name);
    } else if (body.action === 'addAssessmentType') {
      const name=text(body.name).trim(),min=Number(body.minScore),max=Number(body.maxScore);if(!name||!Number.isFinite(min)||!Number.isFinite(max)||max<=min)throw new Error('검사명과 올바른 점수 범위를 입력하세요.');const id=`ASM-${String(Date.now()).slice(-8)}`;db.prepare('INSERT INTO assessment_catalog (id,name,min_score,max_score,active,created_at) VALUES (?,?,?,?,1,?)').run(id,name,min,max,today);logChange(db,'검사 추가','검사',id,null,{name,min,max},`${name} 검사 항목 추가`,user.display_name);
      ...
      const assessment=db.prepare('SELECT min_score,max_score FROM assessment_catalog WHERE id=?').get(text(body.assessmentId)) as {min_score:number;max_score:number}|undefined;if(!assessment)throw new Error('검사 항목을 찾을 수 없습니다.');const pre=body.preScore===''||body.preScore==null?null:Number(body.preScore),postScore=body.postScore===''||body.postScore==null?null:Number(body.postScore);for(const value of [pre,postScore])if(value!==null&&(value<assessment.min_score||value>assessment.max_score))throw new Error(`점수는 ${assessment.min_score}~${assessment.max_score} 범위여야 합니다.`);const before=db.prepare('SELECT * FROM assessment_scores WHERE application_id=? AND assessment_id=?').get(Number(body.applicationId),text(body.assessmentId));const updatedAt=new Date().toISOString();db.prepare(`INSERT INTO assessment_scores (application_id,assessment_id,pre_score,post_score,note,updated_at) VALUES (?,?,?,?,?,?) ON CONFLICT(application_id,assessment_id) DO UPDATE SET pre_score=excluded.pre_score,post_score=excluded.post_score,note=excluded.note,updated_at=excluded.updated_at`).run(Number(body.applicationId),text(body.assessmentId),pre,postScore,text(body.note),updatedAt);logChange(db,'검사 점수','성과검사',`${body.applicationId}:${text(body.assessmentId)}`,before,db.prepare('SELECT * FROM assessment_scores WHERE application_id=? AND assessment_id=?').get(Number(body.applicationId),text(body.assessmentId)),'사전·사후 검사 점수 저장',user.display_name);
    } else if (body.action === 'saveSatisfaction') {
      const score=body.score===''||body.score==null?null:Number(body.score);if(score!==null&&(score<1||score>5))throw new Error('만족도는 1~5점이어야 합니다.');const before=db.prepare('SELECT * FROM satisfaction_surveys WHERE application_id=?').get(Number(body.applicationId)),updatedAt=new Date().toISOString();db.prepare(`INSERT INTO satisfaction_surveys (application_id,score,comment,updated_at) VALUES (?,?,?,?) ON CONFLICT(application_id) DO UPDATE SET score=excluded.score,comment=excluded.comment,updated_at=excluded.updated_at`).run(Number(body.applicationId),score,text(body.comment),updatedAt);logChange(db,'만족도 입력','만족도',Number(body.applicationId),before,db.prepare('SELECT * FROM satisfaction_surveys WHERE application_id=?').get(Number(body.applicationId)),'프로그램 만족도 저장',user.display_name);
      ...
      const destination = source.slice(0,source.length-ext.length)+`-backup-${today}-${Date.now()}${ext}`;
      await backup(db,destination);
      logChange(db,'백업','데이터베이스',path.basename(destination),null,{path:destination},'로컬 데이터 백업 생성',text(body.actor));
      ...
      logChange(db,'백업 점검','데이터베이스',path.basename(text(body.path)),null,result,`백업 파일 무결성 점검: ${result.message}`,text(body.actor));
      ...
      const source=getDatabasePath(),extension=path.extname(source)||'.sqlite',safetyPath=source.slice(0,source.length-extension.length)+`-backup-before-restore-${today}-${Date.now()}${extension}`;
      ...
      try {
        ...
        logChange(db,'복원','데이터베이스',path.basename(restorePath),{safetyBackup:safetyPath},{restoredFrom:restorePath},`백업 복원 완료 · 복원 전 안전 백업 생성`,text(body.actor));
        db.exec('COMMIT');
      ...
      for (const item of input.slice(0,1000)) {
        ...
        if(program) db.prepare(`INSERT OR IGNORE INTO applications (participant_id,program_id,run_id,applied_at,status) VALUES (?,?,NULL,?,'신청')`).run(participant.id,program.id,text(item.appliedAt)||today);
      }
