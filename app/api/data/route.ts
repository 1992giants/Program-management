...
const normalizePhoneDigits = (value:unknown) => text(value).replace(/\D/g,'');
...
function applicationHasHistoricalData(db:DatabaseSync,applicationId:number) {
  return ['attendance','assessment_scores','satisfaction_surveys'].some(table=>Boolean(db.prepare(`SELECT 1 FROM ${table} WHERE application_id=? LIMIT 1`).get(applicationId)));
}
type HistoricalApplication={id:number;run_id:string|null;status:string};
const TERMINAL_APPLICATION_STATUSES=new Set(['참가완료','중도탈락','취소']);
function assertNoTerminalHistoricalReactivation(db:DatabaseSync,application:HistoricalApplication|undefined) {
  if(application&&TERMINAL_APPLICATION_STATUSES.has(application.status)&&applicationHasHistoricalData(db,application.id))throw new ActionError('기존 업무 이력이 있는 종료 신청은 재신청할 수 없습니다.');
}
function requestIp(request:Request){void request;return 'local'}

type ImportProgram={id:string;name:string};
type ImportRun={id:string;program_id:string;label:string};
type ImportResolution<T>={value?:T;code?:string;message?:string};
function resolveProgramForImport(db:DatabaseSync,contextProgram:ImportProgram|undefined,rowProgramName:string):ImportResolution<ImportProgram> {
  if(contextProgram){
    if(rowProgramName&&rowProgramName!==contextProgram.name)return {code:'program_context_mismatch',message:'선택한 프로그램과 Excel 프로그램명이 일치하지 않습니다.'};
    return {value:contextProgram};
  }
  if(!rowProgramName)return {code:'program_required',message:'프로그램명 또는 선택한 프로그램이 필요합니다.'};
  const matches=db.prepare('SELECT id,name FROM programs WHERE name=? LIMIT 2').all(rowProgramName) as ImportProgram[];
  if(!matches.length)return {code:'program_not_found',message:'프로그램을 확인할 수 없습니다.'};
  if(matches.length>1)return {code:'program_ambiguous',message:'같은 이름의 프로그램이 여러 개 있어 가져올 수 없습니다. 프로그램을 직접 선택한 뒤 다시 가져오세요.'};
  return {value:matches[0]};
}
function resolveRunForImport(db:DatabaseSync,contextRun:ImportRun|undefined,program:ImportProgram,runLabel:string):ImportResolution<ImportRun> {
  if(contextRun){
    if(contextRun.program_id!==program.id)return {code:'program_run_mismatch',message:'프로그램과 차수가 일치하지 않습니다.'};
    if(runLabel&&runLabel!==contextRun.label)return {code:'run_context_mismatch',message:'선택한 차수와 Excel 차수명이 일치하지 않습니다.'};
    return {value:contextRun};
  }
  if(!runLabel)return {};
  const matches=db.prepare('SELECT id,program_id,label FROM program_runs WHERE program_id=? AND label=? LIMIT 2').all(program.id,runLabel) as ImportRun[];
  if(!matches.length){if(db.prepare('SELECT 1 FROM program_runs WHERE label=? LIMIT 1').get(runLabel))return {code:'program_run_mismatch',message:'프로그램과 차수가 일치하지 않습니다.'};return {code:'run_not_found',message:'차수를 확인할 수 없습니다.'};}
  if(matches.length>1)return {code:'run_ambiguous',message:'같은 차수명이 여러 개 있어 가져올 수 없습니다. 차수를 직접 선택한 뒤 다시 가져오세요.'};
  return {value:matches[0]};
}
  ...
  try {
    ...
    if(resource==='outcomes'){
      ...
      const assessmentSql=scope.runId===null
        ?`SELECT sc.application_id,sc.assessment_id,sc.pre_score,sc.post_score,sc.pre_date,sc.post_date,sc.note,sc.not_completed_reason FROM assessment_scores sc JOIN applications a ON a.id=sc.application_id LEFT JOIN program_runs r ON r.id=a.run_id WHERE a.program_id=? AND a.status<>'취소' AND (a.run_id IS NULL OR r.program_id=?) ORDER BY sc.application_id,sc.assessment_id`
        :`SELECT sc.application_id,sc.assessment_id,sc.pre_score,sc.post_score,sc.pre_date,sc.post_date,sc.note,sc.not_completed_reason FROM assessment_scores sc JOIN applications a ON a.id=sc.application_id WHERE a.program_id=? AND a.run_id=? AND a.status<>'취소' ORDER BY sc.application_id,sc.assessment_id`;
      const satisfactionSql=scope.runId===null
        ?`SELECT ss.application_id,ss.score,ss.comment,ss.survey_version,ss.anonymous FROM satisfaction_surveys ss JOIN applications a ON a.id=ss.application_id LEFT JOIN program_runs r ON r.id=a.run_id WHERE a.program_id=? AND a.status<>'취소' AND (a.run_id IS NULL OR r.program_id=?) ORDER BY ss.application_id`
        :`SELECT ss.application_id,ss.score,ss.comment,ss.survey_version,ss.anonymous FROM satisfaction_surveys ss JOIN applications a ON a.id=ss.application_id WHERE a.program_id=? AND a.run_id=? AND a.status<>'취소' ORDER BY ss.application_id`;
    ...
    if(resource==='attendance'){
      ...
      const attendance=db.prepare(`SELECT at.id,at.application_id,at.session_id,at.status,at.note,at.contacted_at,at.makeup_for_session_id FROM attendance at JOIN applications a ON a.id=at.application_id JOIN program_runs r ON r.id=a.run_id AND r.program_id=a.program_id WHERE at.session_id=? AND a.run_id=? AND a.program_id=? AND r.id=? AND r.program_id=? AND a.status<>'취소' ORDER BY at.application_id`).all(scope.sessionId,scope.runId,scope.programId,scope.runId,scope.programId);
  ...
  try {
    ...
    if(body.action==='login'){
      ...
      const {secret}=withImmediateTransaction(db,()=>{
        ...
        const session=createAuthSession(db,user.id,ip);
        logChange(db,'로그인','사용자',user.id,null,{role:user.role},'로그인',user.id,'',ip);
        return session;
      });
    ...
    let issuedCertificate:{certificateId:number;participantId:string;applicationId:number;programId:string;runId:string;sessionCount:number}|undefined;
    ...
    if(body.action==='logout'){
      ...
      withImmediateTransaction(db,()=>{db.prepare('UPDATE participants SET name=?,gender=?,age=?,phone=?,member_status=?,note=? WHERE id=?').run(text(body.name),text(body.gender),Number(body.age||0),text(body.phone),text(body.memberStatus),text(body.note),participantId);logChange(db,'수정','참가자',participantId,null,{changed_fields:fields,name_changed:fields.includes('name'),phone_changed:fields.includes('phone'),memo_changed:fields.includes('note')},'참가자 기본정보 수정',user.id,'',requestIp(request))});
      ...
      withImmediateTransaction(db,()=>{db.prepare('UPDATE programs SET name=?,category=?,delivery_type=?,location=?,manager=?,capacity=?,status=? WHERE id=?').run(after.name,after.category,after.delivery_type,after.location,after.manager,after.capacity,after.status,programId);db.prepare('UPDATE sessions SET location=? WHERE run_id IN (SELECT id FROM program_runs WHERE program_id=?)').run(after.location,programId);const canonical=db.prepare('SELECT id,location FROM programs WHERE id=?').get(programId) as {id:string;location:string}|undefined;if(!canonical||canonical.location!==after.location)throw new Error('Program update did not produce a canonical result.');logChange(db,'program_update','프로그램',canonical.id,null,{changed_fields:fields,...(fields.includes('status')?{status_before:String(before.status),status_after:after.status}:{}),name_changed:fields.includes('name'),manager_changed:fields.includes('manager'),location_changed:fields.includes('location'),session_location_updated:affectedSessionCount>0,affected_session_count:affectedSessionCount},'프로그램 정보 수정',user.id,'',requestIp(request))});
    } else if (body.action === 'updateRun') {
      const runId=text(body.id).trim(),label=text(body.label).trim(),run=db.prepare('SELECT id,program_id,label FROM program_runs WHERE id=?').get(runId) as {id:string;program_id:string;label:string}|undefined;
      if(!run)throw new ActionError('차수를 찾을 수 없습니다.');if(!label||label.length>100)throw new ActionError('차수 이름을 1~100자로 입력하세요.');
      if(run.label!==label)withImmediateTransaction(db,()=>{db.prepare('UPDATE program_runs SET label=? WHERE id=?').run(label,run.id);logChange(db,'run_update','차수',run.id,null,{program_id:run.program_id,changed_fields:['label']},'프로그램 차수 이름 수정',user.id,'',requestIp(request))});
    } else if (body.action === 'removeFromRun') {
      const applicationId=Number(body.applicationId),runId=text(body.runId).trim(),application=db.prepare('SELECT id,program_id,run_id FROM applications WHERE id=?').get(applicationId) as {id:number;program_id:string;run_id:string|null}|undefined;
      if(!application||!Number.isInteger(applicationId)||application.run_id!==runId)throw new ActionError('해당 차수에 배정된 신청자를 찾을 수 없습니다.');
      const run=db.prepare('SELECT id,program_id FROM program_runs WHERE id=?').get(runId) as {id:string;program_id:string}|undefined;if(!run||run.program_id!==application.program_id)throw new ActionError('신청자와 차수 범위를 확인하세요.');
      withImmediateTransaction(db,()=>{if(applicationHasHistoricalData(db,application.id))throw new ActionError('기존 출석 또는 결과가 있는 신청은 차수 배정을 해제할 수 없습니다.');db.prepare('UPDATE applications SET run_id=NULL,assigned_at=NULL WHERE id=? AND run_id=?').run(application.id,run.id);logChange(db,'run_unassign','신청',application.id,null,{program_id:application.program_id,run_id:run.id},'차수 배정 해제',user.id,'',requestIp(request))});
    } else if (body.action === 'deleteRun') {
      const runId=text(body.id).trim(),run=db.prepare('SELECT id,program_id,label FROM program_runs WHERE id=?').get(runId) as {id:string;program_id:string;label:string}|undefined;if(!run)throw new ActionError('삭제할 차수를 찾을 수 없습니다.');
      withImmediateTransaction(db,()=>{const assigned=(db.prepare('SELECT COUNT(*) AS count FROM applications WHERE run_id=?').get(run.id) as {count:number}).count,attendance=(db.prepare('SELECT COUNT(*) AS count FROM attendance at JOIN sessions s ON s.id=at.session_id WHERE s.run_id=?').get(run.id) as {count:number}).count;if(assigned||attendance)throw new ActionError('기존 신청 또는 출석 기록이 있는 차수는 삭제할 수 없습니다.');const sessionCount=(db.prepare('SELECT COUNT(*) AS count FROM sessions WHERE run_id=?').get(run.id) as {count:number}).count;db.prepare('DELETE FROM program_runs WHERE id=?').run(run.id);logChange(db,'run_delete','차수',run.id,null,{program_id:run.program_id,unassigned_application_count:assigned,deleted_session_count:sessionCount},'프로그램 차수 삭제',user.id,'',requestIp(request))});    } else if (body.action === 'updateRunStatus') {
    ...
    } else if (body.action === 'updateSessionsBulk') {
      const requestedProgramId=text(body.programId).trim(),requestedRunId=text(body.runId).trim(),items=Array.isArray(body.sessions)?body.sessions as Record<string,unknown>[]:[];
      if(!requestedProgramId||!requestedRunId||items.length<1||items.length>100)throw new ActionError('저장할 회기 변경사항을 올바르게 선택하세요.');
      const run=db.prepare('SELECT id,program_id FROM program_runs WHERE id=?').get(requestedRunId) as {id:string;program_id:string}|undefined;
      if(!run||run.program_id!==requestedProgramId)throw new ActionError('선택한 프로그램과 차수 범위를 확인하세요.');
      const ids=items.map(item=>text(item.id).trim());if(ids.some(id=>!id)||new Set(ids).size!==ids.length)throw new ActionError('회기 정보를 올바르게 선택하세요.');
      const canonical=ids.map(id=>db.prepare('SELECT id,run_id,session_date,session_time,location FROM sessions WHERE id=?').get(id) as {id:string;run_id:string;session_date:string;session_time:string;location:string}|undefined);
      if(canonical.some(session=>!session||session.run_id!==run.id))throw new ActionError('선택한 차수에 속한 회기만 저장할 수 있습니다.');
      const changed=items.map((item,index)=>{const before=canonical[index]!;const after={session_date:text(item.sessionDate),session_time:text(item.sessionTime),location:text(item.location).trim()};if(!isCanonicalCalendarDate(after.session_date)||!/^([01]\d|2[0-3]):[0-5]\d$/.test(after.session_time)||!after.location)throw new ActionError(`${index+1}번째 회기의 날짜, 시간, 장소를 올바르게 입력하세요.`);return {before,after,fields:changedFields(before,after,['session_date','session_time','location'])}}).filter(item=>item.fields.length>0);
      if(changed.length)withImmediateTransaction(db,()=>{const update=db.prepare('UPDATE sessions SET session_date=?,session_time=?,location=? WHERE id=?');for(const item of changed)update.run(item.after.session_date,item.after.session_time,item.after.location,item.before.id);const changedFieldCategories=[...new Set(changed.flatMap(item=>item.fields))];logChange(db,'sessions_bulk_update','차수',run.id,null,{program_id:run.program_id,changed_session_count:changed.length,changed_field_categories:changedFieldCategories},'프로그램 회기 일정 일괄 저장',user.id,'',requestIp(request))});
    } else if (body.action === 'copyRun') {
      const sourceRunId=text(body.sourceRunId).trim(),source=db.prepare('SELECT id,program_id,label,start_date FROM program_runs WHERE id=?').get(sourceRunId) as {id:string;program_id:string;label:string;start_date:string}|undefined;
      if(!source)throw new ActionError('복사할 차수를 찾을 수 없습니다.');
      const sourceSessions=db.prepare('SELECT session_number,session_date,session_time,location FROM sessions WHERE run_id=? ORDER BY session_number').all(source.id) as {session_number:number;session_date:string;session_time:string;location:string}[];
      if(!sourceSessions.length)throw new ActionError('복사할 회기가 없습니다.');
      withImmediateTransaction(db,()=>{const max=(db.prepare('SELECT COALESCE(MAX(round_number),0) AS max_round FROM program_runs WHERE program_id=?').get(source.program_id) as {max_round:number}).max_round,round=max+1,newRunId=`RUN-${source.program_id}-${round}`;db.prepare("INSERT INTO program_runs (id,program_id,round_number,label,start_date,status,closed_at,closed_by) VALUES (?,?,?,?,?,'모집 중',NULL,NULL)").run(newRunId,source.program_id,round,`${source.label} 복사본`,source.start_date);const insert=db.prepare('INSERT INTO sessions (id,run_id,session_number,session_date,session_time,location) VALUES (?,?,?,?,?,?)');for(const session of sourceSessions)insert.run(`${newRunId}-S${session.session_number}`,newRunId,session.session_number,session.session_date,session.session_time,session.location);logChange(db,'run_copy','차수',newRunId,null,{source_run_id:source.id,new_run_id:newRunId,program_id:source.program_id,copied_session_count:sourceSessions.length},'프로그램 차수 복사',user.id,'',requestIp(request))});
    } else if (body.action === 'copyProgram') {
      const sourceProgramId=text(body.sourceProgramId).trim(),name=text(body.name).trim(),source=db.prepare('SELECT id,category,delivery_type,session_count,recurrence,location,manager,capacity FROM programs WHERE id=?').get(sourceProgramId) as {id:string;category:string;delivery_type:string;session_count:number;recurrence:string;location:string;manager:string;capacity:number}|undefined;
      if(!source)throw new ActionError('복사할 프로그램을 찾을 수 없습니다.');if(!name)throw new ActionError('새 프로그램명을 입력하세요.');if(db.prepare('SELECT 1 FROM programs WHERE name=?').get(name))throw new ActionError('같은 이름의 프로그램이 이미 있습니다.');
      const assessments=db.prepare('SELECT assessment_id,sort_order FROM program_assessments WHERE program_id=? ORDER BY sort_order').all(source.id) as {assessment_id:string;sort_order:number}[],newProgramId=`PRG-${randomUUID().slice(0,8)}`;
      withImmediateTransaction(db,()=>{db.prepare("INSERT INTO programs (id,name,category,delivery_type,session_count,recurrence,location,manager,capacity,status,created_at) VALUES (?,?,?,?,?,?,?,?,?,'운영 중',?)").run(newProgramId,name,source.category,source.delivery_type,source.session_count,source.recurrence,source.location,source.manager,source.capacity,today);const insert=db.prepare('INSERT INTO program_assessments (program_id,assessment_id,sort_order) VALUES (?,?,?)');for(const assessment of assessments)insert.run(newProgramId,assessment.assessment_id,assessment.sort_order);logChange(db,'program_copy','프로그램',newProgramId,null,{source_program_id:source.id,new_program_id:newProgramId,copied_run_count:0,copied_session_count:0,copied_assessment_config_count:assessments.length},'프로그램 설정 복사',user.id,'',requestIp(request))});    } else if (body.action === 'updateSession') {
      ...
      const after={session_date:text(body.sessionDate),session_time:text(body.sessionTime),location:text(body.location)};withImmediateTransaction(db,()=>{db.prepare('UPDATE sessions SET session_date=?,session_time=?,location=? WHERE id=?').run(after.session_date,after.session_time,after.location,before.id);logChange(db,'일정 수정','회기',before.id,{session_date:before.session_date,session_time:before.session_time,location:before.location},after,'프로그램 회기 일정 수정',user.id,'',requestIp(request))});
      ...
      withImmediateTransaction(db,()=>{db.prepare('INSERT INTO schedule_events (id,event_type,color,participant_id,title,event_date,all_day,start_time,end_time,recurrence,delivery_mode,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run(id,eventType,color,participantId,title,eventDate,allDay,startTime,endTime,recurrence,deliveryMode,createdAt);logChange(db,'일정 등록','일정',id,null,{event_type:eventType,event_date:eventDate,all_day:Boolean(allDay),start_time:startTime,end_time:endTime,recurrence,delivery_mode:deliveryMode,participant_id:participantId,title_present:Boolean(title)},'일정 등록',user.id,'',requestIp(request))});
      ...
      assertNoTerminalHistoricalReactivation(db,before);
      if(before&&applicationHasHistoricalData(db,before.id))throw new ActionError('기존 업무 이력이 있는 신청은 재신청할 수 없습니다.');
      withImmediateTransaction(db,()=>{db.prepare(`INSERT INTO applications (participant_id,program_id,run_id,applied_at,status,queue_number,status_updated_at) VALUES (?,?,NULL,?,'신청',?,?) ON CONFLICT(participant_id,program_id) DO UPDATE SET status='신청',status_reason='',status_updated_at=excluded.status_updated_at`).run(participant.id,program.id,today,nextQueueNumber(db,program.id),today);const application=db.prepare('SELECT id,participant_id,program_id,run_id,status FROM applications WHERE participant_id=? AND program_id=?').get(participant.id,program.id) as {id:number;participant_id:string;program_id:string;run_id:string|null;status:string};logChange(db,'application_apply','신청',application.id,null,{participant_id:application.participant_id,program_id:application.program_id,run_id:application.run_id,created:!before,reapplied:Boolean(before),status_before:before?.status??null,status_after:application.status},'프로그램 신청 저장',user.id,'',requestIp(request))});
      ...
      if(application.run_id!==run.id&&applicationHasHistoricalData(db,application.id))throw new ActionError('기존 업무 이력이 있는 신청은 다른 차수로 재배정할 수 없습니다.');
      withImmediateTransaction(db,()=>{db.prepare(`UPDATE applications SET run_id=?, status=CASE WHEN status IN ('신청','선정검토') THEN '참가대기' ELSE status END,assigned_at=?,status_updated_at=? WHERE id=?`).run(run.id,new Date().toISOString(),new Date().toISOString(),application.id);const after=db.prepare('SELECT id,run_id,status FROM applications WHERE id=?').get(application.id) as {id:number;run_id:string|null;status:string};logChange(db,'application_assign_run','신청',after.id,null,{run_before:application.run_id,run_after:after.run_id,status_before:application.status,status_after:after.status,assigned:true},'신청 차수 배정',user.id,'',requestIp(request))});
      ...
      assertNoTerminalHistoricalReactivation(db,before);
      if(before&&before.run_id!==run.id&&applicationHasHistoricalData(db,before.id))throw new ActionError('기존 업무 이력이 있는 신청은 다른 차수로 재배정할 수 없습니다.');
      withImmediateTransaction(db,()=>{db.prepare(`INSERT INTO applications (participant_id,program_id,run_id,applied_at,status,queue_number,status_updated_at,assigned_at) VALUES (?,?,?,?,'참가대기',?,?,?) ON CONFLICT(participant_id,program_id) DO UPDATE SET run_id=excluded.run_id,status='참가대기',assigned_at=excluded.assigned_at,status_updated_at=excluded.status_updated_at`).run(participant.id,run.program_id,run.id,today,nextQueueNumber(db,run.program_id),new Date().toISOString(),new Date().toISOString());const application=db.prepare('SELECT id,program_id,run_id,status FROM applications WHERE participant_id=? AND program_id=?').get(participant.id,run.program_id) as {id:number;program_id:string;run_id:string|null;status:string};logChange(db,'application_apply_to_run','신청',application.id,null,{program_id:application.program_id,run_before:before?.run_id??null,run_after:application.run_id,status_before:before?.status??null,status_after:application.status,created:!before,reassigned:Boolean(before&&before.run_id!==application.run_id)},'신청 차수 추가 또는 재배정',user.id,'',requestIp(request))});
      ...
      const applicationId=Number((before as Record<string,unknown>).id);updatedApplicationReason=withImmediateTransaction(db,()=>{db.prepare('UPDATE applications SET status=?,status_reason=?,status_updated_at=? WHERE id=?').run(status,reason,new Date().toISOString(),applicationId);const canonical=db.prepare(`SELECT id,COALESCE(status_reason,'') AS status_reason FROM applications WHERE id=?`).get(applicationId) as {id:number;status_reason:string};logChange(db,'상태 변경','신청',applicationId,{status:(before as Record<string,unknown>).status},{status,reason_present:Boolean(reason)},`신청 현황을 ${status}(으)로 변경`,user.id,'',requestIp(request));return {applicationId:canonical.id,reasonPresent:canonical.status_reason.trim()!=='',statusReason:canonical.status_reason}});
      ...
      const before=db.prepare('SELECT status,note,contacted_at,makeup_for_session_id FROM attendance WHERE application_id=? AND session_id=?').get(applicationId,sessionId) as Record<string,unknown>|undefined;withImmediateTransaction(db,()=>{db.prepare(`INSERT INTO attendance (application_id,session_id,status,note,contacted_at,makeup_for_session_id) VALUES (?,?,?,?,?,?) ON CONFLICT(application_id,session_id) DO UPDATE SET status=excluded.status,note=excluded.note,contacted_at=excluded.contacted_at,makeup_for_session_id=excluded.makeup_for_session_id`).run(applicationId,sessionId,status,noteText,text(body.contactedAt)||null,makeupFor||null);logChange(db,'출석 입력','출석',`${applicationId}:${sessionId}`,{status:before?.status??'미입력'},{status,note_changed:String(before?.note??'')!==noteText,contact_checked:Boolean(text(body.contactedAt)),makeup_changed:String(before?.makeup_for_session_id??'')!==makeupFor},`출석 상태를 ${status}(으)로 변경`,user.id,'',requestIp(request))});
      ...
      db.exec('BEGIN IMMEDIATE');
      try {
        const sourceApps=db.prepare('SELECT id,program_id,run_id FROM applications WHERE participant_id=?').all(mergeId) as {id:number;program_id:string;run_id:string|null}[];
        for(const sourceApp of sourceApps){const targetApp=db.prepare('SELECT id,run_id FROM applications WHERE participant_id=? AND program_id=?').get(keepId,sourceApp.program_id) as {id:number;run_id:string|null}|undefined;if(targetApp){if(sourceApp.run_id!==targetApp.run_id||applicationHasHistoricalData(db,sourceApp.id))throw new ActionError('동일 프로그램에 서로 다른 차수 또는 운영 이력이 있어 자동 병합할 수 없습니다.');const sourceAttendance=db.prepare('SELECT * FROM attendance WHERE application_id=?').all(sourceApp.id) as {session_id:string;status:string;note:string}[];for(const attendance of sourceAttendance){const existing=db.prepare('SELECT status FROM attendance WHERE application_id=? AND session_id=?').get(targetApp.id,attendance.session_id) as {status:string}|undefined;if(!existing)db.prepare('INSERT INTO attendance (application_id,session_id,status,note) VALUES (?,?,?,?)').run(targetApp.id,attendance.session_id,attendance.status,attendance.note);else if(existing.status==='미입력'&&attendance.status!=='미입력')db.prepare('UPDATE attendance SET status=?,note=? WHERE application_id=? AND session_id=?').run(attendance.status,attendance.note,targetApp.id,attendance.session_id);}db.prepare('DELETE FROM applications WHERE id=?').run(sourceApp.id);}else db.prepare('UPDATE applications SET participant_id=? WHERE id=?').run(keepId,sourceApp.id);}
      ...
      const username=text(body.username).trim().toLowerCase(),pin=text(body.pin),role=text(body.role);if(!username||pin.length<6||/^([0-9])\1+$/.test(pin))throw new ActionError('계정명과 반복 숫자가 아닌 6자리 이상 PIN을 입력하세요.');if(!['관리자','일반 담당자','출석 입력 전용'].includes(role))throw new ActionError('올바른 권한을 선택하세요.');const id=`USR-${String(Date.now()).slice(-8)}`,now=new Date().toISOString();withImmediateTransaction(db,()=>{db.prepare('INSERT INTO staff_users (id,username,display_name,pin_hash,role,active,created_at,pin_changed_at,must_change_pin) VALUES (?,?,?,?,?,1,?,?,1)').run(id,username,text(body.displayName),hashPin(username,pin),role,today,now);logChange(db,'계정 등록','사용자',id,null,{role,temporary_pin:true},'직원 계정 생성',user.id,'',requestIp(request))});
      ...
      const name=text(body.name).trim(),min=Number(body.minScore),max=Number(body.maxScore),version=text(body.version)||'1.0',description=text(body.description);if(!name||!Number.isFinite(min)||!Number.isFinite(max)||max<=min)throw new ActionError('검사명과 올바른 점수 범위를 입력하세요.');const id=`ASM-${String(Date.now()).slice(-8)}`;withImmediateTransaction(db,()=>{db.prepare('INSERT INTO assessment_catalog (id,name,min_score,max_score,active,created_at,version,description) VALUES (?,?,?,?,1,?,?,?)').run(id,name,min,max,today,version,description);logChange(db,'검사 추가','검사',id,null,{name,min,max,version},`${name} 검사 항목 추가`,user.id,'',requestIp(request))});
    } else if (body.action === 'setAssessmentActive') {
      const assessmentType=db.prepare('SELECT id,active FROM assessment_catalog WHERE id=?').get(text(body.id)) as {id:string;active:number}|undefined;if(!assessmentType)throw new ActionError('검사 항목을 찾을 수 없습니다.');withImmediateTransaction(db,()=>{db.prepare('UPDATE assessment_catalog SET active=? WHERE id=?').run(body.active?1:0,assessmentType.id);logChange(db,'검사 상태 변경','검사',assessmentType.id,{active:assessmentType.active},{active:body.active?1:0},`검사 항목 ${body.active?'사용':'사용 중지'}`,user.id,'',requestIp(request))});
      ...
      const assessment=db.prepare('SELECT id,min_score,max_score,active FROM assessment_catalog WHERE id=?').get(text(body.assessmentId)) as {id:string;min_score:number;max_score:number;active:number}|undefined;if(!assessment||!assessment.active)throw new ActionError('현재 사용 중인 검사만 저장할 수 있습니다.');
      ...
      const applicationId=Number(body.applicationId),requestedParticipantId=text(body.participantId).trim();
      if(!Number.isInteger(applicationId)||applicationId<=0)throw new ActionError('확인서를 발급할 신청 기록을 선택하세요.');
      issuedCertificate=withImmediateTransaction(db,()=>{
        const application=db.prepare('SELECT a.id,a.participant_id,a.program_id,a.run_id,r.program_id AS run_program_id FROM applications a JOIN program_runs r ON r.id=a.run_id WHERE a.id=?').get(applicationId) as {id:number;participant_id:string;program_id:string;run_id:string;run_program_id:string}|undefined;
        if(!application||application.program_id!==application.run_program_id)throw new ActionError('확인서를 발급할 신청·차수 관계를 확인할 수 없습니다.');
        if(requestedParticipantId&&requestedParticipantId!==application.participant_id)throw new ActionError('선택한 참가자와 신청 기록이 일치하지 않습니다.');
        const sessionCount=(db.prepare("SELECT COUNT(*) AS count FROM attendance at JOIN sessions s ON s.id=at.session_id WHERE at.application_id=? AND s.run_id=? AND at.status IN ('참석','보강')").get(application.id,application.run_id) as {count:number}).count;
        if(sessionCount<1)throw new ActionError('출석 근거를 확인할 수 없어 확인서를 발급할 수 없습니다.');
        const result=db.prepare('INSERT INTO certificates (participant_id,issued_at,session_count) VALUES (?,?,?)').run(application.participant_id,today,sessionCount),certificateId=Number(result.lastInsertRowid);
        ...
        logChange(db,'certificate_create','확인서',certificate.id,null,{participant_id:certificate.participant_id,application_id:application.id,program_id:application.program_id,run_id:application.run_id,session_count:certificate.session_count,issued:true},'참여확인서 발급 이력 생성',user.id,'',requestIp(request));
        return {certificateId:certificate.id,participantId:certificate.participant_id,applicationId:application.id,programId:application.program_id,runId:application.run_id,sessionCount:certificate.session_count};
      });
      ...
      const operationId=`IMPORT-${randomUUID()}`,contextProgramId=text(body.programId).trim(),contextRunId=text(body.runId).trim();
      const suppliedProgram=contextProgramId?(db.prepare('SELECT id,name FROM programs WHERE id=?').get(contextProgramId) as ImportProgram|undefined):undefined;
      if(contextProgramId&&!suppliedProgram)throw new ActionError('선택한 프로그램을 찾을 수 없습니다.');
      const contextRun=contextRunId?(db.prepare('SELECT r.id,r.program_id,r.label,p.name AS program_name FROM program_runs r JOIN programs p ON p.id=r.program_id WHERE r.id=?').get(contextRunId) as (ImportRun&{program_name:string})|undefined):undefined;
      if(contextRunId&&!contextRun)throw new ActionError('선택한 차수를 찾을 수 없습니다.');
      if(suppliedProgram&&contextRun&&suppliedProgram.id!==contextRun.program_id)throw new ActionError('선택한 프로그램과 차수 범위를 확인하세요.');
      const contextProgram=contextRun?{id:contextRun.program_id,name:contextRun.program_name}:suppliedProgram;
      ...
      withImmediateTransaction(db,()=>{
        for(const [index,item] of input.entries()){
          const rowNumber=index+2,name=text(item.name).trim(),rawPhone=text(item.phone).trim(),normalizedPhone=normalizePhoneDigits(rawPhone),rawAge=text(item.age).trim(),rowProgramName=text(item.programName).trim(),runLabel=text(item.runLabel).trim(),appliedAt=text(item.appliedAt).trim(),memberStatus=text(item.memberStatus).trim()||'비회원';
          ...
          const programResolution=resolveProgramForImport(db,contextProgram,rowProgramName);
          if(!programResolution.value){reject(rowNumber,programResolution.code!,programResolution.message!);continue;}
          const program=programResolution.value,runResolution=resolveRunForImport(db,contextRun,program,runLabel);
          if(runResolution.code){reject(rowNumber,runResolution.code,runResolution.message!);continue;}
          const requestedRun=runResolution.value;
          let participant=(db.prepare('SELECT id,phone FROM participants WHERE TRIM(name)=?').all(name) as {id:string;phone:string}[]).find(candidate=>normalizePhoneDigits(candidate.phone)===normalizedPhone) as {id:string}|undefined;
          ...
          const existingApplication=db.prepare('SELECT id,run_id,status FROM applications WHERE participant_id=? AND program_id=?').get(participant.id,program.id) as {id:number;run_id:string|null;status:string}|undefined;
          const resolvedRunId=requestedRun?.id??(existingApplication?.run_id??null);
          try{assertNoTerminalHistoricalReactivation(db,existingApplication)}catch(error){if(error instanceof ActionError){reject(rowNumber,'terminal_historical_reapplication',error.message);continue}throw error}
    ...
    return Response.json({...snapshot(user),...(responseWarning?{warning:responseWarning}:{}),...(updatedApplicationReason?{updatedApplicationReason}:{}),...(importResult?{importResult}:{}),...(issuedCertificate?{issuedCertificate}:{})});
  } catch(error) { return error instanceof ActionError?Response.json({error:error.message},{status:400}):internalError(error); }
}
