...
async function handleGET(request:Request) {
  try { const db=getDatabase(),user=getAuthenticatedUser(request,db);if(!user)return loginRequired();return Response.json(snapshot(user)); }
...
async function handlePOST(request:Request) {
  try {
    ...
    if(body.action==='logout'){
      ...
      const sessionId=text(body.sessionId),applicationId=Number(body.applicationId),status=text(body.status),noteText=text(body.note).trim(),validStatuses=['참석','결석','보강','취소','노쇼','기타','미입력'];if(!validStatuses.includes(status))throw new ActionError('올바른 출석 상태를 선택하세요.');if(['결석','취소','노쇼','기타'].includes(status)&&!noteText)throw new ActionError(`${status} 사유나 연락 결과를 입력하세요.`);const sessionState=db.prepare('SELECT run_id,attendance_status FROM sessions WHERE id=?').get(sessionId) as {run_id:string;attendance_status:string}|undefined;if(!sessionState)throw new ActionError('회기를 찾을 수 없습니다.');if(sessionState.attendance_status==='마감')throw new ActionError('마감된 회기의 출석은 수정할 수 없습니다. 관리자가 회기를 다시 열어야 합니다.');const application=db.prepare('SELECT run_id FROM applications WHERE id=?').get(applicationId) as {run_id:string|null}|undefined;if(!application||application.run_id!==sessionState.run_id)throw new ActionError('해당 차수에 배정된 참가자만 출석을 입력할 수 있습니다.');const makeupFor=text(body.makeupForSessionId);if(makeupFor&&!db.prepare('SELECT id FROM sessions WHERE id=?').get(makeupFor))throw new ActionError('보강 대상 회기를 찾을 수 없습니다.');
      ...
      const name=text(body.name).trim(),min=Number(body.minScore),max=Number(body.maxScore),version=text(body.version)||'1.0',description=text(body.description);if(!name||!Number.isFinite(min)||!Number.isFinite(max)||max<=min)throw new ActionError('검사명과 올바른 점수 범위를 입력하세요.');const id=`ASM-${String(Date.now()).slice(-8)}`;db.prepare('INSERT INTO assessment_catalog (id,name,min_score,max_score,active,created_at,version,description) VALUES (?,?,?,?,1,?,?,?)').run(id,name,min,max,today,version,description);logChange(db,'검사 추가','검사',id,null,{name,min,max,version},`${name} 검사 항목 추가`,user.display_name);
