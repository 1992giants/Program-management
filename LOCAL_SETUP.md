...
일반 담당자의 로그인·새로고침·업무 변경 응답은 관리자 스냅샷과 분리됩니다. 센터명 외 내부 설정 marker, 백업 정보, 잠긴 직원 수, 직원 목록, 확인서 목록, 중복 참가자 상세는 일반 담당자에게 전송하지 않습니다. 참가자와 신청 내역도 화면 운영에 필요한 명시적 컬럼만 조회합니다. 참가자 메모는 초기 응답에서 제외되며, 권한을 확인한 특정 참가자 상세 조회가 성공한 경우에만 한 명분을 불러옵니다.

연락처·성별·나이·회원 여부와 신청 사유는 기존 업무 흐름을 보존하기 위해 초기 응답에 유지합니다. 검사 점수, 만족도 응답, 일정, 출석 상세의 on-demand 조회 전환과 프로그램·차수·회기 컬럼의 추가 최소화는 후속 Sprint 1B-2C 범위로 남아 있습니다.
